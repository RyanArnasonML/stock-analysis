...
# https://medium.com/analytics-vidhya/automating-your-stocks-portfolios-daily-analysis-report-using-python-4f636edd0864
...
while interval < len(list_files):
    ...
    boughtPrice = Data.loc[Data['Date'] == tickers['Purchase Date'][interval]] # stores the stock's price on the day it was purchased
    currentPrice = Data.iloc[[-1]] # stores the stock's last price
    dayBeforePrice = Data.iloc[[-2]] # stores the stock's price 2 weekdays ago

    totalReturn = (currentPrice['Close'].values[0] / boughtPrice['Close'].values[0]) # calculates the stock's total return since purchases by dividing the current price by the purchase price
    totalRet[interval] = totalReturn # adds the 'totalReturn' to the dictionary we created for them

    todaysReturn = (currentPrice['Close'].values[0] / dayBeforePrice['Close'].values[0])  # calculates the stock's return today by dividing the current price yesterday
...
with smtplib.SMTP_SSL("smtp.gmail.com", port, context=cont) as server:
    ...
    server.sendmail("<sender email>", "<receiver email>", email) # sends an email with the body as 'email'
